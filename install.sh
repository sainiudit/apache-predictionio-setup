...
if [[ ! -e ${PIO_FILE} ]]; then
  ...
  #curl -OL https://github.com/apache/incubator-predictionio/archive/develop.zip
  git clone https://github.com/apache/incubator-predictionio.git

  mv develop.zip v${PIO_VERSION}.zip

  #unzip v${PIO_VERSION}.tar.gz


  mv incubator-predictionio PredictionIO-${PIO_VERSION}
  
  bash PredictionIO-${PIO_VERSION}/make-distribution.sh
...
if [[ ! -e spark-${SPARK_VERSION}-bin-hadoop2.7.tgz ]]; then
  ...
  echo https://d3kbcqa49mib13.cloudfront.net/spark-${SPARK_VERSION}-bin-hadoop2.7.tgz
  curl -O https://d3kbcqa49mib13.cloudfront.net/spark-${SPARK_VERSION}-bin-hadoop2.7.tgz
fi
...
case $source_setup in
  ...
  "$ES_HB")
    ...
    if [[ ! -e hbase-${HBASE_VERSION}-bin.tar.gz ]]; then
	  ...
	  echo http://www-eu.apache.org/dist/hbase/${HBASE_VERSION}/hbase-${HBASE_VERSION}-bin.tar.gz
      curl -O http://www-eu.apache.org/dist/hbase/${HBASE_VERSION}/hbase-${HBASE_VERSION}-bin.tar.gz
    fi
